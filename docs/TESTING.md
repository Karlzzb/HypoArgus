...
> 注：上表「测试分层」逐文件计数为历史基线快照，T-04–T-08 新增模块（`test_ws_sender` / `test_translator` / `test_trace_store` / `test_api_run` / `test_ops_service` / `test_session_cache` / `test_redaction` / `test_metrics` / `test_logging_setup` 等）与 SearchAgent V12 适配器 / 真实检索 e2e 套件使总数升至 678。逐文件计数以 `pytest --collect-only -q` 为准（代码实名为唯一真相）。
