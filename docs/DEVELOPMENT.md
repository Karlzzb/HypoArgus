...
> **重构方向（ADR-0017·已落地）**：全部落地（parse+partition 合并、hitl1 重定义、hypothesis_propose、retrieval 桩、judgment 五合一、rewrite_loop 新增 + hitl2 重定位为终稿闸门 + writeback 裁撤）。
...
START → parse+partition
        ...
        → rewrite_loop           （逐段 LLM 提议重写；未触达段逐字节拷回·ADR-0017）
        → hitl2                  （人确认终稿文本·终稿文本确认闸门·重定位）
        → final_document
END
...
| 新增 | `rewrite_loop` | 逐段 LLM 提议重写；产 `proposed_rewrites`。ADR-0017 |
| 重定位 | `hitl2` | 终稿文本确认闸门：逐段确认 / 编辑 / 驳回 `proposed_rewrites`，拼 `final_document`。ADR-0017 |
| 裁撤 | `writeback` | 终稿在 hitl2 落地，不再有独立回写节点。ADR-0017 |
...
└────────────────────────────────────────────────────────────────────────────┘
  ...
  ▼
┌────────────────────────────────────────────────────────────────────────────┐
│ ⑥ rewrite_loop       【单次·逐段调 LLM·有 _guarded·ADR-0017】       │
...
> 五合一删除了原 ReAct 独占 infra：`infra/retrieval_tool.py` / `infra/tool_protocol.py` / `infra/history.py`（RetrievalTool / ToolRegistry / HistoryStore）——五合一后 judgment 吃预取 citations、不再有 ReAct 循环与步间历史。
...
| `agents/rewrite_loop/{contract,agent}.py` | `propose_rewrites(argument_tree, citations, paragraph_list, sc, qtr, llm) -> RewriteLoopOutcome`（逐段提议重写·ADR-0017） | `RewriteLlmClient` / `FakeRewriteLlmClient` | 被触达段（supported 假说 / 命中 citations） |
...
> **重构方向节点增删**（ADR-0017 / ADR-0026）：全部落地（parse+partition 合并、hitl1 重定义、hypothesis_propose、retrieval 桩→真实后端、judgment 五合一 + 删除 verification ReAct 模块与独占 infra、rewrite_loop 新增、hitl2 重定位为终稿闸门、writeback 裁撤）。
...
## 6. judgment 的 partial 合流（ADR-0017 五合一）

五合一后检索之后的五节点（verification ReAct 取证 / hypothesis 取证 / merge / impact / consistency）并入单一 `judgment` 节点。
...
因 judgment 是检索之后唯一的整树写者、直接把终态写回树，故裁撤原 `argument_credibility` partial channel（ADR-0017 终局）——避免多线路整节点 upsert 互相覆盖丢字段（§11 铁律：共享可变状态换成带 reducer 的 channel）。
...
pytest -q                     # 基线（删 writeback 测试 + 加 rewrite_loop/hitl2 测试）
```
...
- **段落原文永不被 LLM 改写**：段落原文 `original_content`（存于 `ParagraphRecord`，ADR-0025）只来自只读字节表解码、`Argument` 不再存原句字段（by construction）。rewrite_loop 对**被触达段**产**提议重写文本**（写入 `proposed_rewrites`、不回写 `ParagraphRecord.original_content`），最终是否落地由 hitl2 人确认——这是 ADR-0017 对「终稿逐字节一致 / 段落原文不被 LLM 改写」承诺的受控放宽（仅被触达段、需人拍板；未触达段仍逐字节忠实）。
- **绝不替人拍板**：judgment（含 merge/impact/consistency）不置 `adopted`、不改 `content`。重构后 `adopted`/`corrected`/`adopted_hypothesis_id` 在新流程不再被写（domain 字段保留不删）；终稿文本确认只经 hitl2 逐段确认 / 编辑 / 驳回 `proposed_rewrites`。
...
源自原 `docs/langgraph-dev-guide.md`（已并入本文、文件移除）。
本节收口贯穿设计的几条 durable 规则；具体节点装配见 §1、状态机见 §7、扩展点见 §8。
原 dev-guide 的 DeepResearchState / supervisor / Send fan-out 骨架是「深度研究」范式示例，与本管线固定单向图不同，故不并入——只留与实现无关的铁律。
（原文件 `docs/langgraph-dev-guide.md` 已删除。）

- **控制流落边，不落 prompt 散文**：Agent 是一张状态图，LLM 每步只做极窄的结构化决策（要不要继续、调哪个工具），「接下来执行什么」由图的拓扑 + 条件边决定，不由模型散文决定。
  凡是能靠 prompt「请你务必…」被模型忽略的规则，都要落成图里可执行的边或校验。
- **结构化输出 + 校验重试**：用 `with_structured_output(schema)` 让 SDK 层保证结构，拿到即合法对象；校验失败在 node 里 catch，把错误拼进下次 prompt 重调或用条件边路由回同一 node。
  各 LLM seam 的扁平 BaseModel 信封（`ParseResult` / `_ProposalsEnvelope` / `JudgmentResult` / `_RewriteEnvelope`）即此落地。
- **共享可变状态换成带 reducer 的 state channel**：LangGraph 里几乎不该手写 `asyncio.Lock`；并行 worker 只 `return` partial state，由 reducer 无锁合并。
  `merge_argument_tree`（整树 upsert、首见顺序、同 id 覆盖）与 `_merge_dict` 即此模式——避免多线路整节点 upsert 互相覆盖丢字段。
- **interrupt + Command(resume) 三条坑**（异步 HITL 待落地，ADR-0022）：① 必须配 `checkpointer`，否则无法冻结 state；② 恢复时整个 node 从头重跑（不是从 `interrupt()` 那行继续）——别把 interrupt 放在有副作用的代码后面；③ 多个 interrupt 按顺序索引匹配 resume 值，别在节点里条件跳过 interrupt。
- **失败要么被修复、要么被如实标注，绝不静默吞或伪装成功**：图该有 `errors` / `status` 的 state key 让下游与前端看得见降级。
  `_guarded` 兜底 + `errors` channel（reducer `_append_errors`）即此落地；`Hitl2GateError` 是正确性硬停、不兜底。
