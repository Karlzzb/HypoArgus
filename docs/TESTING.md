...
| 异常兜底 | `test_orchestrator_fallback.py` | 任一 stage 异常 → 兜底 + 单向推进 + 日志（PRD §13） | 10 |
...
合计 **387 passed, 3 skipped**（390 collected；skip：`test_writeback.py:330` 样例不足两段 ×2；`test_real_llm_wiring.py:235` dashscope_smoke 需 key+网络 ×1）。
Slice 6 后裁撤 `writeback` 节点：`test_writeback.py` 重写为 rewrite_loop propose + hitl2 assemble 纯函数子缝（提议→确认→拼接）、`test_hitl2.py` 重写为段落级三态、`test_orchestrator_resume.py` 重写为 `resume_rewrite` 幂等拼装；新增 rewrite_loop 装配 / 触达确认 e2e 测试。故基线较 Slice 5（399 passed）下降至 387 passed（删旧 writeback / resume 测试 + 加新 rewrite_loop / hitl2 测试净减）。
...
### `test_orchestrator_fallback.py`（10）
