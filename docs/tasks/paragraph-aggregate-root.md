...
| T-04 | 翻转：移除 Argument.paragraph_id/content，退役 paragraph_summaries，真实 LLM 测试适配 | T-02, T-03 | 已完成 |
| T-05 | 文档（STATE.md / CONTEXT.md）+ 新 ADR 取代 ADR-0005 决策 2 | T-04 | 已完成 |
...
- [x] `src/domain.py::Argument` 移除 `paragraph_id` 与 `content`，仅保留推理字段。
- [x] `PipelineState` 移除 `paragraph_summaries` channel 与其 reducer；`paragraph_list.summary` 为摘要单一定义点。
  - `ParseOutput.paragraph_summaries` 字段同步退役（摘要折叠进 `paragraph_list.summary`）；`ParseResult.paragraph_summaries`（LLM seam 输出）保留不动。
- [x] `argument_tree` channel 与 `merge_argument_tree` reducer 不变；`OriginalParagraphs` 不动。
- [x] `assemble_final_document` / `resume_rewrite` 不变；`build_review` 左栏原文仍取 `OriginalParagraphs`（与解码 bytes 等价、layer-1 真相源），`paragraph_list.original_content` 为同源等价路径。
- [x] `tests/test_real_llm_parse.py` 的字节保护断言由节点级 `content.encode(surrogateescape) == op.get(pid)` 改为段级 `original_content` 对照（契约 2 段级、契约 3 全覆盖、契约 7 `paragraph_list.summary`）。真实 paper_04 验证通过。
- [x] orchestrator e2e（`tests/test_orchestrator_e2e.py`）断言 tracer-bullet 字节一致 + 段落↔节点一致性不变式（经 HITL-1 编辑后仍一致）。真实 paper_03 全流水线 byte-identical 通过。
- [x] Fake-LLM spy 断言「改写 seam 收到该段 `original_content`」（T-02 建，本切片复用，未改）。
- [x] 全仓无残留对 `Argument.paragraph_id` / `Argument.content` / `paragraph_summaries` 的读取（grep 核实——余下仅为 partition `Paragraph.content`（bytes）、`ParagraphRecord.paragraph_id` 与「不再读」措辞的文档性提及）。
- [x] 质量门全绿。
  - `ruff check src tests` ✓；`mypy --strict src` ✓（54 源文件）；`pytest -q -m "not real_llm"` ✓（633 passed, 3 skipped, 11 deselected）。
  - 真实 LLM 单流程：paper_03 全流水线（parse→开药→裁决→重写提议→hitl2）byte-identical（67s）；paper_04 真实解析契约（53s）。
...
- [x] `docs/STATE.md` 以 `paragraph_list` 为字段流向单一定义点，`paragraph_summaries` 相关描述移除，`Argument` 移除字段在 §2 反映，各 seam 输入 / 子智能体契约在 §3/§4 反映。
  - §1 主表新增 `paragraph_list` 行（reducer `merge_paragraph_list`）+ 段落聚合根 channel 类别；§1.1 `OriginalParagraphs` 读者改为 parse / hitl2（rewrite_loop 经 `paragraph_list.original_content` 取原文）；§1.2 移除 `paragraph_summaries` 行与类别（退役注折叠进 §1）；§2 移除 `paragraph_id` / `content` 行 + 新增 §2.3 `ParagraphRecord`（含一致性自检硬停说明）；§3 节点速查表与 §3.2 边界改读 `paragraph_list`（签名同步：`propose_hypotheses` / `judge_and_adjudicate` / `propose_rewrites` / `consistency` / `confirm_partition` 收 `paragraph_list`）；§4 四 seam 输入由 `argument.content` 改为段 `original_content`、judgment 按段聚合。
- [x] `CONTEXT.md`「实现映射」更新，无 `Argument.content` / `paragraph_id` 残留措辞。
  - L38 解析 Agent 产出改 `paragraph_list`；新增「段落聚合根 (ParagraphRecord / paragraph_list)」概念条目；段落摘要条目改为承载于 `ParagraphRecord.summary`（退役 `paragraph_summaries` channel）。
- [x] ADR-0001 / 0009 / 0017 中提及 `Argument.content` / `paragraph_id` 的措辞同步更新。
  - ADR-0001 背景：节点不再携带 `paragraph_id` 外键，归属经 `ParagraphRecord.argument_tree_ids`（ADR-0025）；ADR-0009：LLM 不回填节点 `paragraph_id` 指针、归属由段落聚合根正向承载；ADR-0017：`content` 措辞改为段落原文 `original_content`（存于 `ParagraphRecord`）。ADR-0005 状态标注决策 2 被 ADR-0025 取代。
- [x] 新增 ADR（编号顺延）取代 ADR-0005 决策 2，rationale 如实记录 ADR-0017 段级化使原 rationale 失效；layer-1 字节表与「不整篇进 Agent 上下文」不变式标注保持不动。
  - `docs/adr/0025-paragraph-as-aggregate-root.md`：记录「节点存原句 → 段落存原句」翻转、两 channel + id 引用方案（vs 嵌套单 channel 变体）、HITL-1 同步义务 + 一致性自检硬停；layer-1 字节表与「不整篇进 Agent 上下文」不变式标注不动。
- [x] 未改动 `CHANGELOG.md` 或任何 auto-generated 文件。
- [x] 质量门全绿（文档变更无代码影响，仍核验无回归）。
  - `ruff check src tests` ✓；`mypy --strict src` ✓；`pytest -q -m "not real_llm"` ✓。真实 LLM 单流程（一段文章改写，DashScope）已跑通。
