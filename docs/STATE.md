...
| `paragraph_summaries` | `dict[str, str]`（`paragraph_id → 摘要`） | `_merge_dict`（单写者无冲突） | `【LLM】` `parse+partition` 同一次调用顺产 | hypothesis_propose / rewrite_loop | ADR-0021；Slice 1。**不并入 `OriginalParagraphs`**（保字节级无损只读表身份，ADR-0005/0009） |
...
- **parse+partition**：partition 产 `original_paragraphs`（只读原文表）+ 空树（纯代码、字节级自检失败硬停）；parse 建初始树，铸 `argument_id` / `parent_id` / `children_ids` / `paragraph_id` / `content`(拷回) / `argument_weight`(clamp) / `status=unverified`。无提议段落降级为 `background` 影子节点。
...
> 四条 seam 的 contract schema 均为扁平 BaseModel（`ParseResult` / `_ProposalsEnvelope` / `JudgmentResult` / `_RewriteEnvelope`、无判别联合 `oneOf`），故直接用 contract schema 经 `with_structured_output` 绑定、无需扁平信封映射。
