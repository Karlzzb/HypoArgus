...
def plan_gap_retrieval(task, *, gap_reason: str | None, missing_slots: list[str], round_number: int = 0) -> GapRetrievalPlan:
    if round_number >= 1 or gap_reason not in ALLOWED_GAP_REASONS:
    ...
    if gap_reason == "INSUFFICIENT_INDEPENDENT_SOURCES":
        queries.append(normalize_query_preserving_numbers(
            " ".join(part for part in (years, regions, subject, metrics, numbers, "官方 研究机构 报告") if part)
        ))
    elif gap_reason in {"NO_EVIDENCE", "INSUFFICIENT_EFFECTIVE_COUNT"} and not queries:
        queries.append(normalize_query_preserving_numbers(
            " ".join(part for part in (years, regions, subject, metrics, numbers, "官方 报告") if part)
        ))
