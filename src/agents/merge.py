...
def apply_partial_updates(
    ...
    若让二者直接写同一 ``argument_tree`` channel、由整节点 upsert reducer 折叠，则后写者会
    整节点覆盖先写者——``status`` 与 ``candidate_hypotheses`` 无法在同节点共存，合并算子
    无从读 ``原文.status × 假设.status`` 矩阵。本函数按字段归属合流，消除该竞态。
    """

    out: list[Argument] = []
    for argument in argument_tree:
        update: dict[str, object] = {}
        credibility = argument_credibility.get(argument.argument_id)
        if credibility is not None:
            update["status"] = credibility
        hyps = hypotheses.get(argument.argument_id)
        if hyps is not None:
            update["candidate_hypotheses"] = hyps
        out.append(argument.model_copy(update=update))
    return out


# 合并算子对外形态：``(argument_tree, argument_tree, argument_tree) -> argument_tree``（将两线路 partial 视为同棵树的不同字段切片）。
MergeLike = Callable[[list[Argument]], list[Argument]]
...
def merge_with_partials(
    ...
    combined = apply_partial_updates(argument_tree, argument_credibility, hypotheses)
...
_VERDICT_ROWS: frozenset[ArgumentStatus] = frozenset(
    {ArgumentStatus.CREDIBLE, ArgumentStatus.DOUBTFUL, ArgumentStatus.ERROR}
)
...
def _decide(
    ...
    hypotheses = list(argument.candidate_hypotheses)

    # 未裁决节点（体检未覆盖、或并行未跑完）→ 保守 KEEP、不裁剪、不激活。
    if argument.status not in _VERDICT_ROWS:
        return MergeDecision(action=MergeAction.KEEP), hypotheses, []

    if argument.status is ArgumentStatus.CREDIBLE:
        return _decide_credible(hypotheses)
    return _decide_flawed(hypotheses)


def merge(argument_tree: list[Argument]) -> list[Argument]:
    ...
    out: list[Argument] = []
    for argument in argument_tree:
        decision, kept_hypotheses, extra_tags = _decide(argument)
        new_tags = list(argument.issue_tags)
