...
| 模块 | 职责 |
|---|---|
| `infra/retrieval.py` | `RetrievalLayer` Protocol + Mock + 合规校验（白名单 / 权限 / 模板在接口层强制）。体检 / 开药共用。 |
| `infra/retrieval_tool.py` | `RetrievalTool` 包 `RetrievalLayer`，独占 `SearchStep → RetrievalRequest` 翻译（ADR-0015）。 |
| `infra/tool_protocol.py` | 工具框架 seam 骨架（`ToolRegistry` / `BaseTool` / `ToolResult`，ADR-0015）。 |
| `infra/history.py` | `HistoryStore`：ReAct 步间检索观察记忆 + 压缩（ADR-0016）。 |
| `infra/llm_provider.py` | `build_qwen_chat_model()`：`ChatOpenAI` 指向 DashScope 端点；API key 只读 `DASHSCOPE_API_KEY`。 |
| `infra/llm_adapters.py` | `QwenParseLlmClient` / `QwenVerifyLlmClient` / `QwenHypothesisLlmClient`：经 `with_structured_output` 绑各 seam 契约。 |
...
| 模块 | 接口 | seam | 覆盖节点 |
|---|---|---|---|
| `agents/parser/{contract,agent}.py` | `parse(original_paragraphs, llm) -> list[Argument]` | `LlmClient` / `FakeLlmClient` | 全段（唯一读段落文本的环节） |
| `agents/hitl1/{contract,agent}.py` | `confirm(argument_tree, gate) -> list[Argument]` | `Hitl1Gate` / `FakeHitl1Gate` | 结构编辑（可跳过） |
| `agents/verification/{contract,agent}.py` | `verify(argument_tree, llm, retrieval, *, max_iterations) -> dict[str, ArgumentStatus]` | `VerifyLlmClient` / `FakeVerifyLlmClient` | main_claim / sub_claim / evidence |
| `agents/hypothesis/{contract,agent}.py` | `hypothesize(argument_tree, llm, retrieval, *, max_iterations) -> dict[str, list[Hypothesis]]` | `HypothesisLlmClient` / `FakeHypothesisLlmClient` | evidence / sub_claim（不读体检结论，ADR-0002） |
| `agents/hitl2/{contract,agent}.py` | `confirm(argument_tree, original_paragraphs, gate)` / `build_review(...)` | `Hitl2Gate` / `FakeHitl2Gate` / `ConservativeHitl2Gate` | 待决节点（不可跳过硬闸门，ADR-0010） |
| `agents/merge.py` | `merge` / `merge_with_partials` / `apply_partial_updates` | 无（确定性 12 格矩阵纯函数） | 全节点标注 |
| `agents/impact.py` | `impact(argument_tree) -> list[Argument]` | 无（串行·不产文本·剩余支撑率纯函数） | 上层论点 invalid/weakening |
| `agents/consistency.py` | `consistency(argument_tree) -> list[Argument]` | 无（单次扫描·只贴 `issue_tags`） | 段落级 / 全局一致性 |
| `agents/writeback.py` | `writeback(argument_tree, original_paragraphs) -> WritebackResult` | 无（段落原子缝合·幂等纯函数） | 被采纳节点 |
...
- **content 永不被 LLM 改写**：节点文本只来自只读表逐字节拷回（`agents/parser/agent.py` 先例，by construction）。
