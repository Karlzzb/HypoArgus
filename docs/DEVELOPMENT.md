...
数据在 stage 间经 **state channel** 路由，**无跨模块直接调用**；流水线严格单向——唯 `hitl1` 经条件边有**有界打回**（`hitl1 → parse+partition`，max retries 默认 3，超限向前 + 贴 `partition_retry_exhausted`；ADR-0018），其余 stage 绝不打回。
...
START → parse+partition
        ...
        → judgment               （LLM 取证 + 纯函数 merge/impact/consistency·ADR-0019）
        → rewrite_loop           （逐段 LLM 提议重写；未触达段逐字节拷回·ADR-0017·Slice 6）
        → hitl2                  （人确认终稿文本·终稿文本确认闸门·Slice 6 重定位）
        → final_document
END
贯穿 state：session_context(session/user/current_time/user_prompt) + query_time_range（ADR-0021）
```
...
│    gate.review(argument_tree) → SKIP / ACCEPT / EDIT / REPLAY（ADR-0018）   │
│    SKIP/ACCEPT/EDIT = 确认继续（EDIT 逐 op validate_tree；改树形不改文本）  │
│    REPLAY = 打回重跑 parse+partition（按 user prompt·当前伪代码桩 ADR-0020）│
...
└────────────────────────────────────────────────────────────────────────────┘
  │
  │  hitl1 出条件边（ADR-0018）：route=CONTINUE → 默认下游（hypothesis_propose）；
...
└────────────────────────────────────────────────────────────────────────────┘
  ...
  ▼
┌────────────────────────────────────────────────────────────────────────────┐
│ ⑤ judgment             【单次·有 _guarded·有 LLM·五合一·ADR-0019】          │
...
> **重构方向节点增删**（ADR-0017~0021）：Slice 1–6 全部落地（parse+partition 合并、hitl1 重定义、hypothesis_propose、retrieval 桩、judgment 五合一 + 删除 verification ReAct 模块与独占 infra、rewrite_loop 新增、hitl2 重定位为终稿闸门、writeback 裁撤）。
...
## 6. judgment 的 partial 合流（ADR-0019·Slice 5 五合一）
...
因 judgment 是检索之后唯一的整树写者、直接把终态写回树，故裁撤原 `argument_credibility` partial channel（ADR-0019 终局）——避免多线路整节点 upsert 互相覆盖丢字段（dev-guide §2.2 铁律：共享可变状态换成带 reducer 的 channel）。
