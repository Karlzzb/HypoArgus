...
from .schemas import ForwardItem, KnowledgeContext, ParagraphSearchInput, ReverseItem, StrictModel
...
class OrganizationContext(StrictModel):
    ...
    def reject_blank_identifiers(self):
        for name in ("school_id", "tenant_id"):
        ...
        for result in self.results:
            for citation_id in result.citation_ids:
                ...
                if result.task_id not in citation.task_ids:
                    raise ValueError(f"citation {citation_id} is not bound to task {result.task_id}")
...
__all__ = [
    "INPUT_SCHEMA_VERSION", "OUTPUT_SCHEMA_VERSION", "AgentRunStatus",
    "AtomicClaimDecision", "CitationJudgment", "CitationProvenance",
    "CitationRecord", "EvidenceGap", "OrganizationContext", "ParagraphInput", "PublicWarning",
    "SearchAgentGraphState", "SearchAgentInputState", "SearchAgentOutputState",
    "TaskDecision", "TraceReference",
]
