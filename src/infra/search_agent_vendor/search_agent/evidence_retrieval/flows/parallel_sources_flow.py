...
from ..schemas import (
    ClaimJudgeResult, ErrorDetail, EvidenceCandidate, EvidenceItem, EvidenceQuality, EvidenceRelation, ExecutionStatus,
    JudgeResult, LineType, PreparedContext, QueryItem, RetrievalTask, RetrievalTaskResult,
    SourceRef, SourceType, TerminationReason, ToolUsage, VerificationResult,
    VerificationVerdict, canonical_url, stable_json_hash,
    source_evidence_fingerprint, stable_evidence_item_key,
)
...
from ..scope_guard import apply_claim_scope_guard, apply_scope_guard
...
def build_web_query_variants(task: RetrievalTask, primary: QueryItem | None = None) -> list[QueryItem]:
...
def build_verification_reason(
    ...
    if gap_reason == "MISSING_REQUIRED_SLOTS":
        return f"当前证据未能覆盖所有必需信息槽位，因此暂不能形成确定结论。"
...
def _metrics() -> dict[str, Any]:
    ...
    calls = ["llm_batch_evidence_judge", "llm_batch_judge_repair", "web_search", "web_fetch", "selected_kb_retrieve", "public_kb_retrieve", "structured_query"]
    return {
        "total_elapsed_ms": 0,
        "stage_timings_ms": {key: 0 for key in stages},
        "call_counts": {key: 0 for key in calls},
        "cache": {"query_cache_hits": 0, "web_content_cache_hits": 0},
        "web_task": {},
        "web_request_funnel": {
        ...
        "scope_guard": {"checked_count": 0, "mismatch_count": 0, "mismatch_reasons": {}},
...
class ParallelSourcesFlow:
    def __init__(self, config: EvidenceRetrievalConfig, dependencies: EvidenceRetrievalDependencies, trace: SafeTraceEmitter, shared_cache: dict[str, Any] | None = None):
        ...
        self.kb_semaphore = asyncio.Semaphore(config.public_kb_concurrency)
        self.web_semaphore = asyncio.Semaphore(max(2, config.batch_concurrency))
    ...
    async def _prepare_structured_tool_calling(self, tasks: list[RetrievalTask]) -> None:
        ...
        for (paragraph_id, paragraph_tasks), value in zip(grouped.items(), values):
    ...
    async def _cached_fetch(self, task: RetrievalTask, candidate: EvidenceCandidate) -> FetchResult:
        ...
        if url in self.content_cache:
            ...
            if future is None:
                future = asyncio.create_task(self._measured_call("web_fetch", self.deps.web_fetcher.fetch(candidate.model_copy(update={"task_id": "parallel-shared"}))))
    ...
    async def _web(self, task: RetrievalTask, query: QueryItem) -> tuple[list[EvidenceCandidate], list[ErrorDetail]]:
        ...
        queries = [query] if ":gap:" in query.query_id else build_web_query_variants(task, query)
        metadata = {
        ...
        try:
            ...
            async with self.trace.span("web.fetch", {**metadata, "url_count": len(selected)}) as span:
                ...
                for original, value in zip(selected, fetched):
                    if isinstance(value, BaseException):
    ...
    async def _kb(
        ...
        *,
        ...
        query_variants = build_kb_query_variants(task)
        if query_override:
            ...
            try:
                async def retrieve_variant(variant: dict[str, str]):
                    ...
                    async with self.kb_semaphore:
                        return variant, await self._measured_deadline_call(
                            f"{label}_retrieve",
                            self.deps.kb_client.retrieve(
                                knowledge_ids=ids, query=variant["query"],
                                top_k=self.config.bisheng_retrieve_top_k,
                                score_threshold=self.config.bisheng_retrieve_score_threshold,
                                user_id=task.user_id, origin=origin,
                            ),
    ...
    async def _rank(self, task: RetrievalTask, candidates: list[EvidenceCandidate]) -> list[EvidenceCandidate]:
        metadata = {
            ...
            "judge_ready_candidate_count": len(candidates),
        ...
        async with self.trace.span("candidate.merge", metadata, parent_run_id=parent_run_id) as span:
            started = time.monotonic()
            # Sort every unique candidate; top_k=len guarantees ranking cannot
            # silently become a candidate budget.
            ranked = _lexical_rank(
                task.target_text, candidates, top_k=len(candidates),
            ...
            span["output"] = {"ranked_count": len(ranked), "final_candidate_count": len(ranked)}
    ...
    async def _judge_direction(self, retrieved):
        ...
        for task, query, candidates, errors, usage, selected_ok, structured_status, task_metrics, task_span in retrieved:
        ...
        if any(rows for _, rows, _ in groups):
            ...
            async def run_batch(batch: JudgeBatch):
                ...
                self.metrics["call_counts"]["llm_batch_evidence_judge"] += 1
                async with batch_semaphore:
                    provider_started = time.monotonic()
                    batch_record["queue_wait_ms"] = int(
                        (provider_started - batch_started) * 1000
                    )
                    async with self.trace.generation(
        ...
        gap_results = await asyncio.gather(*(
            ...
            if result_by_task[task.task_id].evidence_gap in {
                "NO_EVIDENCE", "MISSING_REQUIRED_SLOTS", "INSUFFICIENT_EFFECTIVE_COUNT",
            ...
            async with self.gap_judge_semaphore:
                ...
                async with self.trace.generation(
                    ...
                    {"request_id": task.request_id, "task_id": task.task_id, "batch_id": batch.batch_id,
                     ...
                     "estimated_input_tokens": batch.estimated_input_tokens},
                    model=self.config.judge_model or "unavailable", provider="openai_compatible",
    ...
    async def run(self, tasks: list[RetrievalTask], scenarios: dict[str, Any]) -> tuple[list[RetrievalTaskResult], dict[str, Any]]:
        ...
        try:
            await self._prepare_structured_tool_calling(tasks)
            ...
            judged_results = await self._judge_direction(retrieved)
            by_task = {row.task_id: row for row in judged_results}
            ...
            for paragraph_id, pair in grouped.items():
                for forward, reverse in zip(pair["forward"], pair["reverse"]):
