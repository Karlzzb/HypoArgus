"""线路 2 · 开药 Agent：仅投机生成（PRD §5、issue #5、重构）。
...
重构：hypothesis 节点重定义为 hypothesis_propose——仅 propose、不取证、产 pending
假说。原 ``hypothesize``（propose + verify）改名为 ``propose_hypotheses``；取证步模型
（verdict / search / conclude / verify-step）移除（推迟到 judgment seam）。
"""
