...
def redact(value: Any, config: EvidenceRetrievalConfig, key: str = "") -> Any:
    ...
    if isinstance(value, str):
        ...
        for secret in (config.volcano_api_key, config.bisheng_token, config.structured_token):
...
class SafeTraceEmitter:
    ...
    async def generation(
        ...
        *, model: str | None = None, provider: str | None = None,
        ...
        span_data: dict[str, Any] = {"_child_id": child_id, "_started": started_at}
        observation_metadata = _build_metadata(safe, self.config, self.sanitizer)
        # Prune inputs: never persist the full prompt or candidate content.
        inputs: dict[str, Any] = {
            "request_id": safe.get("request_id"),
            "task_id": safe.get("task_id"),
            "item_id": safe.get("item_id"),
            "line_type": safe.get("line_type"),
            "chunk_size": safe.get("chunk_size"),
        }
