...
二者在 ``merge`` 处 join。``tree`` channel 用 ``merge_tree`` reducer 无锁合并两线路
的 partial 更新（dev-guide §2 铁律：共享可变状态换成带 reducer 的 channel）。
...
def merge_tree(
    ...
    right: list[ArgumentationNode] | dict[str, ArgumentationNode] | None,
) -> list[ArgumentationNode]:
    """``tree`` channel 的 reducer：按 ``node_id`` upsert，保持首见顺序。

    - parse/merge/impact/consistency/hitl2 返回整树（``list``）：同 id 覆盖、新 id 追加。
    - verification/hypothesis 返回 partial 更新（``dict[node_id, node]``）：仅 upsert。

    并行两线路各自只 return partial state，reducer 折叠——绝不手写锁
    （dev-guide §2.2 迁移铁律）。
    """
    ...
    if isinstance(right, dict):
        ...
        updates = {n.node_id: n for n in (right or [])}
    for nid, node in updates.items():
        pos = index.get(nid)
        if pos is None:
            index[nid] = len(merged)
            merged.append(node)
        else:
            merged[pos] = node
    return merged
...
class PipelineState(TypedDict, total=False):
    ...
    真相源，Agent 不整篇加载）；``tree`` 为论证树（带 reducer 合并）；``final_doc``
    为终稿 bytes；``errors`` 记录异常兜底日志（#11 接入）。
    """
...
class Orchestrator:
    ...
    def _build_graph(self) -> Any:
        ...
        def verification_node(state: PipelineState) -> dict[str, object]:
            """线路 1 · 体检（#4 接入 ReAct）。返回 partial 更新。"""

            return {"tree": agents.verification(state["tree"])}

        def hypothesis_node(state: PipelineState) -> dict[str, object]:
            """线路 2 · 开药（#5 接入）。返回 partial 更新。"""

            return {"tree": agents.hypothesis(state["tree"])}

        def merge_node(state: PipelineState) -> dict[str, object]:
            """双轨合并算子（#6 接入 12 格矩阵）。"""

            return {"tree": agents.merge(state["tree"])}
