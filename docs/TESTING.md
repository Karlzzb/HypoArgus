...
| 真实论文 LLM | `test_real_llm_parse.py` / `test_real_llm_pipeline_e2e.py`（`real_papers.py` 载 9 篇 `markdown/*.md`） | 真实 DashScope 解析 9 篇真实论文（8 条行为契约）+ 2 篇 E2E 终稿逐字节还原；`real_llm` 标记、无 key 整模块 skip | 11 |

合计 **617 collected**：离线 **603 passed / 3 skipped**（`-m "not real_llm"`）+ 真实论文 LLM **11**（`real_llm` 标记、默认 deselected、需 `DASHSCOPE_API_KEY` + 网络）。
skip：`test_writeback.py:330` 样例不足两段 ×2；`test_real_llm_wiring.py:293` dashscope_smoke 需 key+网络 ×1。
> 注：上表「测试分层」为 T-03 基线；T-04–T-08 新增模块（`test_ws_sender` / `test_translator` / `test_trace_store` / `test_api_run` / `test_ops_service` / `test_session_cache` / `test_redaction` / `test_metrics` / `test_logging_setup` 等）与真实论文 LLM 套件使总数升至 617。逐文件计数以 `pytest --collect-only -q` 为准（代码实名为唯一真相）。
...
pytest -q                         # 含真实论文 LLM（~20min、需 DASHSCOPE_API_KEY + 网络）
