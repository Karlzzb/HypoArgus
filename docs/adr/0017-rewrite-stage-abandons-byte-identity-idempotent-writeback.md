...
- 节点 `content` 永不被 LLM 改写（`CONTEXT.md` 实现映射；`agents/parser/agent.py` 逐字节从只读表拷回，by construction）。
...
1. **重写阶段（被触达段）放弃**：
   - 「终稿逐字节一致」；
   - 「`content` 永不被 LLM 改写」——被触达段的终稿文本由 LLM 提议重写（`rewrite_loop` 节点）；
...
- 放弃「`content` 永不被 LLM 改写」换取基于全文证据的连贯重写能力——这是新产品方向的核心诉求，机械子串替换无法实现。
