...
from .public_contracts import (
    ...
    CitationRecord, EvidenceGap, PublicWarning, SearchAgentInputState,
    SearchAgentOutputState, TaskDecision, TraceReference,
)
from .schemas import (
    EvidenceItem, EvidenceRelation, RetrievalTaskResult, SearchAgentBatchInput,
    SourceType, build_retrieval_tasks, stable_json_hash,
)
...
def to_internal_batch_request(value: SearchAgentInputState) -> SearchAgentBatchInput:
    ...
    return SearchAgentBatchInput(
        ...
        organization_context=value.organization_context.model_dump(mode="json") if value.organization_context else None,
    )
...
def _quote_mode(item: EvidenceItem) -> str:
    ...
    if item.snippet_only:
        return "SNIPPET"
    return "NORMALIZED" if str(item.metadata.get("quote_match_mode") or "").casefold() == "normalized" else "EXACT"
...
_METRIC_PATTERNS = (
    (re.compile(r"市场规模|market[_\s-]?size", re.I), re.compile(r"市场规模|market[_\s-]?size", re.I)),
    (re.compile(r"出货量|shipment", re.I), re.compile(r"出货量|shipment", re.I)),
    (re.compile(r"同比|环比|增长率|复合增长|CAGR|growth", re.I), re.compile(r"同比|环比|增长率|复合增长|CAGR|growth", re.I)),
    (re.compile(r"份额|占比|market[_\s-]?share", re.I), re.compile(r"份额|占比|market[_\s-]?share", re.I)),
    (re.compile(r"保有量|inventory", re.I), re.compile(r"保有量|inventory", re.I)),
    (re.compile(r"招聘|接收人数|实习生人数|actual[_\s-]?count", re.I), re.compile(r"招聘|接收人数|实习生人数|actual[_\s-]?count", re.I)),
...
def _citation(
    ...
    supported = list(item.metadata.get("supported_claim_ids") or []) if item.relation == EvidenceRelation.SUPPORT else []
    refuted = list(item.metadata.get("refuted_claim_ids") or []) if item.relation == EvidenceRelation.REFUTE else []
    ...
    if item.relation in {EvidenceRelation.SUPPORT, EvidenceRelation.REFUTE}:
        ...
        if item.scores.directness < config.public_citation_min_directness:
            return None
        if any(not _complete_fact_sentence(content, claim_text_by_id[claim_id]) for claim_id in mapped_ids):
            return None
    ...
    retrieved_at = str(item.metadata.get("retrieved_at") or datetime.now(timezone.utc).isoformat().replace("+00:00", "Z"))
...
def build_public_output(
    ...
    for result in results:
        ...
        for claim in task.atomic_claims:
            claim_citations = [
                row for row in task_citations
                if claim.claim_id in (row.judgment.supported_claim_ids + row.judgment.refuted_claim_ids)
            ]
            ...
            atomic.append(AtomicClaimDecision(
        ...
        for raw_warning in web_task_metrics.get(result.task_id, {}).get("warnings", []):
            ...
            warning = PublicWarning(code=code, message=str(raw_warning), task_ids=[result.task_id], retryable=True)
            public_warnings[(warning.code, warning.message)] = warning

        public_claim_verdicts = {row.claim_id: row.verdict for row in atomic}
        logic_verdict = apply_claim_logic(list(public_claim_verdicts.values()), task.claim_logic_operator)
        ...
        decisive_and_refuted = task.claim_logic_operator.value == "AND" and logic_verdict == "REFUTED"
        ...
        if _force_downgrade:
            ...
            for acr in atomic:
                if acr.verdict in {"SUPPORTED", "REFUTED"}:
                    ...
                    _downgraded_atomic.append(AtomicClaimDecision(
        ...
        decisions.append(TaskDecision(
            ...
            verdict=verdict,
            confidence=result.verification.confidence if verdict != "INCONCLUSIVE" else min(result.verification.confidence, 0.59),
            conclusion_summary=_conclusion_summary(verdict, task.target_text, atomic, decisive_and_refuted),
            ...
            evidence_gap=EvidenceGap(
                reason=(_gap_reason := result.evidence_gap) if result.evidence_gap != "CONFLICTING_EVIDENCE" or (len(supporting) + len(refuting) + len(supplementary)) > 0 else "INSUFFICIENT_EFFECTIVE_COUNT",
                missing_slots=result.evidence_quality.missing_slots,
                gap_retrieval_triggered=result.gap_retrieval_triggered,
                resolved=result.gap_resolved,
            ) if result.evidence_gap else None,
            warnings=list(dict.fromkeys(warning_codes)),
        ))

    structured_warnings = diagnostic.get("flow_metrics", {}).get("structured_tool_calling", {}).get("warnings", [])
    ...
    status = "ERROR" if errors and not completed and not partial else "PARTIAL" if partial or errors else "SUCCESS"
...
__all__ = ["to_internal_batch_request", "adapt_v11_input_to_legacy", "build_public_output"]
