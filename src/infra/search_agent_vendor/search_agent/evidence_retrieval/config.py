...
class EvidenceRetrievalConfig(BaseModel):
    ...
    public_kb_concurrency: int = Field(default=8, ge=1, le=32)
    ...
    structured_token: SecretStr | None = Field(default=None, repr=False)
    ...
    structured_timeout_ms: int = Field(default=3000, ge=100)
    ...
    judge_batch_max_tasks: int = Field(default=1, ge=1, le=24)
    judge_batch_max_candidates: int = Field(default=6, ge=1, le=60)
    judge_batch_max_input_tokens: int = Field(default=30000, ge=1000, le=200000)
    judge_batch_concurrency: int = Field(default=2, ge=1, le=4)
    ...
    structured_min_tool_calls: int = Field(default=1, ge=0, le=12)
    ...
    def from_env(cls, **overrides: Any) -> "EvidenceRetrievalConfig":
        # Be robust when consumers import the module directly rather than the
        # package root: the repository/package-local .env remains the single
        # configuration source for test and production.
        try:
        ...
        names = {
            ...
            "structured_base_url": "STRUCTURED_BASE_URL", "structured_token": "STRUCTURED_TOKEN",
