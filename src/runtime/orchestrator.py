...
:func:`merge_with_partials` 字段级合流到同一棵树再整树写入 ``tree``——避免并行
...
异常兜底与单向流控（issue #11 · PRD §13）：每个下游 stage 经 :func:`_guarded` 统一兜底，
任一智能体异常 / 超时即「就地置目标节点错误状态 + 附日志 + 单向向前推进」，绝不因单点波动
卡死整篇。硬约束：无复杂分布式重试降级与跨模块挂起——异常即记日志、就地降级、继续向前。
:class:`Hitl2GateError` 为硬闸门正确性硬停（绝不无人拍板自动采纳，ADR-0010），非单点
波动，**不兜底、原样上抛**。

**拓扑 seam（PipelineSpec）**：流水线拓扑由数据驱动的 :class:`StageSpec` 序列描述、
由 :meth:`Orchestrator._build_graph` 据此布线。默认 :func:`default_pipeline` 复刻上述
固定拓扑；调用方可传入另一种 spec（如省略 ``hypothesis`` / ``consistency``）以表达不同
拓扑——这是该 seam 的「第二种 adapter」，使其成为真 seam 而非假 seam（deep-module
原则：two adapters means a real seam）。各 stage 的 *实现* 仍由 :class:`Agents`
逐个注入、可独立插拔（adapter 层 seam）。
"""
...
def merge_tree(
    ...
    :func:`merge_with_partials` 字段级合流后整树写入 ``tree``——避免并行两线路
...
def default_pipeline() -> tuple[StageSpec, ...]:
    """默认流水线拓扑：复刻 PRD §13 固定顺序的 10 个 stage 与并行 / join 边。

    partition → parse → hitl1 → (verification ∥ hypothesis) → merge → impact →
    consistency → hitl2 → writeback → END。
    """

    return (
        StageSpec("partition", _partition_node, ()),
        StageSpec("parse", _parse_node, ("partition",)),
        StageSpec("hitl1", _hitl1_node, ("parse",)),
        StageSpec("verification", _verification_node, ("hitl1",)),
        StageSpec("hypothesis", _hypothesis_node, ("hitl1",)),
        StageSpec("merge", _merge_node, ("verification", "hypothesis")),
        StageSpec("impact", _impact_node, ("merge",)),
        StageSpec("consistency", _consistency_node, ("impact",)),
        StageSpec("hitl2", _hitl2_node, ("consistency",)),
        StageSpec("writeback", _writeback_node, ("hitl2",)),
    )
