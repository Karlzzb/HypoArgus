...
class StructuredQueryClient:
    ...
    async def scenarios(self, force: bool = False) -> dict[str, StructuredScenario]:
        if not force and self._scenario_cache and self._scenario_cache[0] > time.monotonic():
        ...
        self._scenario_cache = (time.monotonic() + self.config.structured_scenarios_cache_ttl_seconds, scenarios)
        return scenarios
