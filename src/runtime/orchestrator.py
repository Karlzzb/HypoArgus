...
（``argument_credibility`` / ``hypotheses``），由 ``merge_node`` 经
:func:`agents.merge.merge_with_partials` 字段级合流到同一棵树再整树写入 ``argument_tree``——避免并行
...
from domain import Argument, ArgumentStatus, Hypothesis
...
def merge_argument_tree(
    ...
    ``argument_tree`` 只承载**整树**写入（parse / merge / impact / consistency / hitl2 各返回
    一棵完整树）：同 id 覆盖、新 id 追加。体检 / 开药的 partial 更新走各自的专用
    channel（``argument_credibility`` / ``hypotheses``），由 ``merge_node`` 经
    :func:`agents.merge.merge_with_partials` 字段级合流后整树写入 ``argument_tree``——避免并行两线路
...
class PipelineState(TypedDict, total=False):
    ...
    整树写入）；``argument_credibility`` / ``hypotheses`` 为两线路 partial 更新（字段级合流
    于 merge 节点，避免并行整节点覆盖）；``final_document`` 为终稿 bytes；``errors`` 记录
    异常兜底日志（#11 接入）。
    """
    ...
    argument_credibility: Annotated[dict[str, ArgumentStatus], _merge_dict]
...
class Orchestrator:
    ...
    def run(
        self, original_doc: bytes, *, session_config: dict[str, Any] | None = None
    ) -> bytes:
        ...
        return self.run_with_report(original_doc, session_config=session_config).final_document
    ...
    def run_with_report(
        ...
        result: dict[str, Any] = self.graph.invoke(
            {"original_doc": bytes(original_doc)}, config=session_config
        )
    ...
    def resume_writeback(
        ...
        return self.agents.writeback(list(argument_tree), original_paragraphs)
