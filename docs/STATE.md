...
reducer 见 `runtime/orchestrator.py`（`merge_argument_tree` / `merge_paragraph_list` / `_merge_dict` / `_append_errors`）。

| 字段 | 类型 | reducer | 来源（创建者） | 谁读 | 证据 |
|---|---|---|---|---|---|
| `original_doc` | `bytes` | — | `【用户】` 入口注入 | partition | `runtime/orchestrator.py` invoke；`runtime/run_real.py` |
| `original_paragraphs` | `OriginalParagraphs` | — | `【代码】` partition 构造 | parse / hitl2（皆只读旁路；rewrite_loop 经 `paragraph_list.original_content` 取原文、不直读字节表） | `agents/assembly.py` parse+partition 闭包；`original_paragraphs.py` |
| `argument_tree` | `list[Argument]` | `merge_argument_tree`（按 `argument_id` upsert 整树） | `【代码】` partition 初始化 `[]`；`【LLM】` parse 建初始树；`【HITL】` hitl1 EDIT（merge/split/mark_no_op/reparent/set_type，检索前）；`【LLM】` judgment 整树写回（检索后唯一整树写者：吃 `citations` 判终态 + 按序调 merge/impact/consistency 纯函数） | parse→hitl1→hypothesis_propose→retrieval→judgment→rewrite_loop→hitl2 全程 | 写点见 §3 各子智能体；reducer `runtime/orchestrator.py` `merge_argument_tree` |
| `paragraph_list` | `list[ParagraphRecord]` | `merge_paragraph_list`（按 `paragraph_id` upsert、保持首见段序） | `【代码】` parse+partition 产（按 `OriginalParagraphs.paragraph_ids()` 规范段序、每段一条；`original_content` 取自该段 bytes 解码、`summary` 折叠自 `ParseResult.paragraph_summaries`、`argument_tree_ids` 覆盖该段全部节点含 background 影子）；`【HITL】` hitl1 EDIT 时同步 `argument_tree_ids` 写回（SKIP/ACCEPT/REPLAY 原样深拷） | hitl1 / hypothesis_propose / judgment / rewrite_loop / consistency / CLI 渲染 / checkpoint（皆经 `argument_id → ParagraphRecord` 反查取该段原文 + 摘要） | ADR-0025；写 `agents/parser/agent.py` + `agents/hitl1/agent.py`；reducer `runtime/orchestrator.py` `merge_paragraph_list` |
...
- **整树 channel**（`argument_tree`）：多写者、带 `merge_argument_tree` reducer，按 `argument_id` 同 id 覆盖、新 id 追加。
  五合一后 judgment 为检索之后的**唯一**整树写者（吃 `hypotheses` / `citations` partial 判终态、再按序调 merge/impact/consistency 纯函数后整树写回），故裁撤 `argument_credibility` partial channel（ADR-0017）。
  重构后 hitl2 不再写 `argument_tree`（按段落 / 文本工作、与 argument 状态解耦），`argument_tree` 在 judgment 之后冻结——rewrite_loop 只读用于触达判定；hitl2 不碰 `argument_tree`（只读 `original_paragraphs` + `proposed_rewrites` 拼终稿）。
...
下表为流水线重构（ADR-0017）所接受的契约。已全部落地
（`session_context` / `query_time_range` / `citations` / `partition_retry_count` /
`hitl1_route` 均已并入 `PipelineState`；`proposed_rewrites` 已并入 §1 主表，
...
| 字段 | 类型 | reducer | 来源（创建者） | 谁读 | 证据 |
|---|---|---|---|---|---|
| `session_context` | `SessionContext`（`session_id` / `user_id` / `current_time: datetime` / `user_prompt`） | —（单写者，无冲突） | `【用户】` 入口注入（`runtime/run_real.py`，与 `original_doc` 同入 START） | retrieval / judgment / hypothesis_propose / rewrite_loop（全链只读，进 LLM 背景） | ADR-0017 |
| `query_time_range` | `TimeRange`（`start: date \| None` / `end: date \| None` / `rationale: str`） | —（单写者） | `【代码】` `parse+partition` 注入桩值（`TimeRange(start=2025, end=2026, rationale="默认值·真实识别待后续")`，不真实调 LLM） | retrieval / judgment / rewrite_loop | ADR-0017（桩），真实识别 Out of Scope |
| `citations` | `dict[str, list[Source]]`（key 为 `argument_id` 如 `n0001`/`bg-...` 或 `hypothesis_id` 如 `h-...`，两套 id 不冲突） | `_merge_dict`（单写者无冲突） | `【代码】` retrieval（真实后端已落地：vendored SearchAgent V12，ADR-0026；未注入 `retrieval_runtime` 时退桩产空 citations） | judgment / rewrite_loop | ADR-0017；ADR-0026（真实后端已接） |
| `partition_retry_count` | `int` | —（单写者，覆盖） | `【代码】` hitl1 build 闭包（打回 +1；超限不 +1） | hitl1 自身（循环续传计数） | ADR-0017 |
| `hitl1_route` | `str`（`"continue"` / `"replay"`） | —（单写者，覆盖） | `【代码】` hitl1 build 闭包（据 `confirm_partition` 产 `Hitl1Outcome.route`） | hitl1 条件边 `_hitl1_route`（ADR-0017 受控打回边） | ADR-0017 |

> `proposed_rewrites`（rewrite_loop）已并入 §1 主表（单写者=rewrite_loop、读者=hitl2、reducer=`_merge_dict`）。
...
> 原 `writeback` 产出的 `final_document` 已改由 hitl2 拼装落地（ADR-0017），`writeback` 节点裁撤——已落地。终稿拼装幂等续跑入口为 `Orchestrator.resume_rewrite(resolved_rewrites, original_paragraphs)`。
...
段落原文与段落↔节点归属**不在 `Argument` 上**——`Argument` 是纯推理结构（无 `paragraph_id` / `content`），原句每段一份存于段落聚合根 `ParagraphRecord`（见 §2.3 / ADR-0025）。
...
| `status` | `ArgumentStatus` | `【LLM】` judgment 取证终判（`credible/doubtful/error`）；`【代码】` impact 判 `invalid` | 否（不进任何 LLM） | judgment `agents/judgment/agent.py`；impact `agents/impact.py`。**注**：重构后 `adopted`/`corrected` 在新流程（rewrite_loop + hitl2 终稿闸门）不再被写——hitl2 按段 / 文本工作、与 argument 状态解耦；`adopted`/`corrected`/`adopted_hypothesis_id` 等 domain 字段保留不删（minimizing domain churn） |
| `issue_tags` | `list[str]` | `【代码】` merge 追加 `conflict`；`【代码】` consistency 扫贴；`【代码】` impact 追加 `weakening`（三者经 judgment 串联调用） | 否 | `agents/merge.py`；`agents/consistency.py`；`agents/impact.py`。**注**：原 writeback 失败贴 `writeback_error` 已随 writeback 裁撤移除 |
| `candidate_hypotheses` | `list[Hypothesis]` | `【LLM】` hypothesis_propose 产 pending 假说；`【LLM】` judgment 取证落终态后经 merge 裁剪存活 | 否（propose 只把 `argument_type` + 段 `original_content` + `summary` 进 prompt；judgment 只把假说 `text` 进 prompt；rewrite_loop 只读用于触达判定、不回灌 `candidate_hypotheses`） | `agents/hypothesis/agent.py`；`agents/judgment/agent.py`；`agents/rewrite_loop/agent.py` |
...
| `adopted_hypothesis_id` | `str \| None` | （新流程不写；原 `【HITL】` hitl2 `AdoptOp` 立即持久化，随 hitl2 重定位移除） | 否 | domain 字段保留不删 |
...
**注**：新流程（rewrite_loop + hitl2 终稿闸门）按段落 / 文本工作、与 argument 的 `status` / `merge_decision` 解耦——`adopted` / `corrected` / `adopted_hypothesis_id` 在新流程不再被写（domain 字段保留不删）；终稿拼装的幂等续跑入口改为 `Orchestrator.resume_rewrite(resolved_rewrites, original_paragraphs)`（按段文本重推导，不再依赖 `adopted_hypothesis_id`）。
...
### 2.3 `ParagraphRecord`（`paragraph_list` channel 的元素，`domain.py`）

`ParagraphRecord` 定义于 `src/domain.py`，是 `paragraph_list` channel 的元素——段落聚合根，正向拥有与论证节点的一对多关系（ADR-0025）。
按 `OriginalParagraphs.paragraph_ids()` 规范段序、每段一条（含被降级为 background 影子节点的段），与字节表完全对齐。

| 字段 | 类型 | 来源（写入者） | 进 LLM? | 证据 |
|---|---|---|---|---|
| `paragraph_id` | `str` | `【代码】` parse 从 `OriginalParagraphs.paragraph_ids()` 对齐 | 否 | `agents/parser/agent.py` |
| `summary` | `str` | `【LLM】` parse 摘要阶段（`ParseResult.paragraph_summaries` 折叠）——摘要单一定义点（退役的 `paragraph_summaries` channel 并入此处） | 是（hypothesis / rewrite 进 prompt 的段落摘要） | `agents/parser/agent.py`；`infra/llm_adapters.py` |
| `original_content` | `str` | `【代码】` parse 取该段 `OriginalParagraphs.get(pid)` 解码文本（`surrogateescape`，每段唯一一份；**LLM 无权改写**） | 是（hypothesis / judgment / rewrite 进 prompt 的核心原文输入，取代原 `Argument.content`） | `agents/parser/agent.py`；`infra/llm_adapters.py` |
| `argument_tree_ids` | `list[str]` | `【代码】` parse 按段覆盖该段全部节点（核心 + background 影子）；HITL-1 `merge` / `split` / `mark_no_op` 同步维护 | 否 | `agents/parser/agent.py`；`agents/hitl1/agent.py` |

> **一致性自检（硬停）**：`argument_tree` 中每个 `argument_id` 恰出现于一个段落的 `argument_tree_ids`、且 `argument_tree_ids` 中每个 id 都存在于 `argument_tree`，不符即硬停（遵循「正确性硬停、不兜底」惯例，ADR-0025）。
> 「同段才能合并」断言由 `argument_tree_ids` 归属判定（取代原 `Argument.paragraph_id` 比较）。
...
| retrieval | `argument_tree` / `hypotheses` / `query_time_range` / `session_context` / `paragraph_list` | `citations` | 否（retrieval 无 LLM seam；真实后端 vendored V12 `with_llm=False` 跑、judgment 重判，ADR-0026） | `retrieval(argument_tree, hypotheses, qtr, sc, paragraph_list) -> dict[str, list[Source]]` `agents/assembly.py`（桩）/ `agents/retrieval/agent.py:real_retrieval`（真实） |
...
> **judgment 内部串联**（纯函数、逻辑不动）：`judge_and_adjudicate` 先 `llm.judge(...)` 取 per-argument / per-hypothesis 终态（按段聚合节点 + 段原文一次），构造局部 `argument_credibility` + 终态化 `hypotheses`，再按序调 `merge_with_partials` → `impact` → `consistency`（`consistency` 收 `paragraph_list`、按 `argument_tree_ids` 分组 + `original_content` 去重），整树写回 `argument_tree`（单写者，故裁撤 `argument_credibility` partial channel）。
> merge/impact/consistency 均为确定性纯函数、无 LLM / 检索依赖，**不在拓扑中独立成 stage**——它们的串联编排收口于 judgment 节点（ADR-0017）。
> rewrite_loop 只读 `argument_tree`（触达判定）+ `paragraph_list`（按 `argument_tree_ids` 解析段内节点、取该段 `original_content`）用于 LLM 输入，**不写 `argument_tree`**（与 argument 状态解耦，失败信号落 `errors` channel + 段回退原文）。hitl2 只读 `original_paragraphs` + `proposed_rewrites`，拼装 `final_document`，亦不写 `argument_tree`。

### 3.2 各子智能体可改字段（一句话边界）

- **parse+partition**：partition 按 ATX 标题边界 + 空行切分产 `original_paragraphs`（只读原文表；P-05：零空行文档按标题边界切、不塌成单段，字节级不变）+ 空树（纯代码、字节级自检失败硬停）；parse 只喂 `is_substantive` 段（去纯 `---` 主题分隔线，P-02）建初始树，铸 `argument_id` / `parent_id` / `children_ids`（n-id 按幸存提议连续赋值、空位 parent 落根，P-04）/ `argument_weight`(clamp) / `status=unverified`；同点产 `paragraph_list`（每段一条，`original_content` 取自该段 bytes 解码、`summary` 折叠自 `ParseResult.paragraph_summaries`、`argument_tree_ids` 覆盖该段全部节点）。无提议段落（含被 `is_substantive` 排除的 `---` 段）降级为 `background` 影子节点。
- **hitl1**：人审结构编辑。可改 `argument_id` / `parent_id` / `children_ids` / `argument_type` / `argument_weight`；`merge` / `split` / `mark_no_op` 同步维护所属段 `argument_tree_ids`（ADR-0025），`reparent` / `set_type` 不动成员关系。「同段才能合并」由 `argument_tree_ids` 归属判定。
- **hypothesis_propose**：只产候选假设 `list[Hypothesis]`（`status=pending`）；覆盖范围 `evidence/sub_claim`。propose 不取证、不读检索（ADR-0002 解法 A）。逐 argument 经 `argument_tree_ids` 反查该段 `original_content` + `summary` 喂 `propose`（不读 `Argument` 原文字段）。
- **judgment**：吃 `citations` 判 per-argument / per-hypothesis 终态。落 `status`（原文侧 `credible/doubtful/error`）+ `Hypothesis.status`（`pending`→终态）；经 merge/impact/consistency 串联写 `merge_decision` / `issue_tags` / 裁剪 `candidate_hypotheses` / 翻上层 `invalid`；覆盖范围 `main_claim/sub_claim/evidence`。不置 `adopted`。
...
> parse 按段喂、不整篇 dump；hypothesis 注 `argument_type` + 该段 `original_content` + `summary`；judgment 注 `argument_type` + 段 `original_content`（按段聚合节点、每段原文一次）+ 假说 `text` + citation 片段 + `session_context` / `query_time_range` 背景；rewrite 注 `summary` + 段 `original_content` + 段内 `argument.argument_type` + 段内假说 `text` / `relation` + citation 片段 + 运行背景。
