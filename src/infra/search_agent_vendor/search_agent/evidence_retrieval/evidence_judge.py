...
from .schemas import (
    ClaimJudgeResult, ErrorDetail, EvidenceCandidate, EvidenceItem, EvidenceRelation, EvidenceScores,
    JudgeResult, NeutralReason, PreparedContext, RetrievalTask, source_evidence_fingerprint,
    SourceType, stable_evidence_item_key, stable_evidence_key,
)
...
class EvidenceJudge(Protocol):
    async def judge(self, task: RetrievalTask, candidate: EvidenceCandidate, context: PreparedContext) -> JudgeResult: ...
...
class BatchEvidenceJudge(Protocol):
    async def judge_many(self, groups: list[tuple[RetrievalTask, list[EvidenceCandidate], PreparedContext]]) -> dict[tuple[str, str], JudgeResult]: ...
...
def validate_judgement(candidate: EvidenceCandidate, result: JudgeResult) -> JudgeResult:
    ...
    for quote in result.quoted_spans:
        ...
        return JudgeResult(
            relation=EvidenceRelation.NEUTRAL, confidence=min(result.confidence, 0.25), directness=0,
            reason="Judge quote was not present in the candidate content.", quoted_spans=[],
            covered_slots=[], missing_slots=result.missing_slots, neutral_reason=NeutralReason.QUOTE_NOT_FOUND,
        )
...
class DeterministicEvidenceJudge:
    ...
    async def judge(self, task: RetrievalTask, candidate: EvidenceCandidate, context: PreparedContext | None = None) -> JudgeResult:
        ...
        return JudgeResult(
            relation=relation, confidence=min(0.55, overlap), directness=0.2 if relation == EvidenceRelation.SUPPLEMENT else 0,
            reason="Conservative lexical fallback; no direct factual direction inferred.",
            quoted_spans=[quote] if quote else [], covered_slots=[], missing_slots=task.required_slots,
        )
...
class StructuredLLMBatchEvidenceJudge:
    ...
    def _diagnostic(self, groups, extraction: ExtractedLLMPayload, *, phase: str, parse_errors=None) -> dict[str, Any]:
    ...
    def _errors_for_tasks(
        self, groups, output: dict[tuple[str, str], JudgeResult], parse_errors: list[dict[str, Any]],
        *, empty: bool = False, repair_error: str | None = None, repair_timed_out: bool = False,
        ...
        compact_reason = "; ".join(str(row.get("reason", "invalid item")) for row in parse_errors[:3])
        for task_id, count in expected.items():
            task_valid = valid.get(task_id, 0)
            relevant_errors = [
                row for row in parse_errors
                if not row.get("warning") and (not row.get("task_id") or str(row.get("task_id")) == task_id)
            ]
            relevant_reason = "; ".join(str(row.get("reason", "invalid item")) for row in relevant_errors[:3])
            ...
            rows = [ErrorDetail(
                code=code.value, node="batch_judge", tool="llm_batch_evidence_judge",
                retryable=code in {ErrorCode.JUDGE_EMPTY_RESPONSE, ErrorCode.JUDGE_VALIDATION_ERROR},
                reason=reason,
            )]
            if repair_error:
                if repair_timed_out:
                    rows.append(ErrorDetail(
                        code=ErrorCode.JUDGE_TIMEOUT.value, node="batch_judge_repair",
                        tool="llm_batch_evidence_judge", retryable=True,
                        reason="Batch Evidence Judge 格式修复重试超时。",
                    ))
                rows.append(ErrorDetail(
                    code=ErrorCode.JUDGE_REPAIR_RETRY_ERROR.value, node="batch_judge_repair",
                    tool="llm_batch_evidence_judge", retryable=False,
                    reason=repair_error,
                ))
    ...
    async def judge_many(self, groups):
        ...
        try:
            ...
            output, parse_errors = parse_batch_judge_response(extraction.payload, candidates, claims_by_task)
            ...
            extraction = ExtractedLLMPayload(
                payload="", raw_response_type=f"{type(raw).__module__}.{type(raw).__name__}",
                raw_response_length=0,
                raw_response_preview=_safe_preview(raw, self.config.batch_judge_raw_preview_chars),
                provider_response_metadata={}, empty=False, recognized=False,
            )
            output = {}
            parse_errors = [{
                "stage": "parse", "reason": f"响应提取/解析内部异常：{type(exc).__name__}"
            }]
        diagnostics = [self._diagnostic(groups, extraction, phase="initial", parse_errors=parse_errors)]
        ...
        should_repair = (
            not output and bool(candidates) and bool(parse_errors)
            and (extraction.empty or any(row.get("stage") in {"parse", "container"} for row in parse_errors))
        ...
        if should_repair:
            ...
            try:
                ...
                repaired = extract_llm_payload(repaired_raw, self.config.batch_judge_raw_preview_chars)
                repaired_output, repaired_errors = parse_batch_judge_response(repaired.payload, candidates, claims_by_task)
                diagnostics.append(self._diagnostic(groups, repaired, phase="repair_retry", parse_errors=repaired_errors))
                diagnostics[-1]["repair_retry_result"] = "SUCCESS" if repaired_output else "INVALID"
                ...
                diagnostics.append({"phase": "repair_retry", "repair_retry_result": type(exc).__name__})
        ...
        errors_by_task = self._errors_for_tasks(
            groups, output, parse_errors,
            empty=extraction.empty, repair_error=repair_error, repair_timed_out=repair_timed_out,
        )
        ...
        if missing_keys:
            ...
            for task_id, candidate_id in missing_keys:
                errors_by_task.setdefault(task_id, []).append(ErrorDetail(
                    code=ErrorCode.JUDGE_VALIDATION_ERROR.value,
                    node="batch_judge", tool="llm_batch_evidence_judge",
                    retryable=True,
...
class SingleJudgeBatchAdapter:
    ...
    async def judge_many(self, groups):
        import asyncio
        inputs = [(task, candidate, context) for task, rows, context in groups for candidate in rows]
        values = await asyncio.gather(*(self.judge.judge(*item) for item in inputs), return_exceptions=True)
        output = {
            (task.task_id, candidate.candidate_id): validate_judgement(candidate, value)
            for (task, candidate, _), value in zip(inputs, values) if isinstance(value, JudgeResult)
        }
        errors_by_task: dict[str, list[ErrorDetail]] = {}
        for (task, _, _), value in zip(inputs, values):
            if isinstance(value, JudgeResult):
                continue
            errors_by_task.setdefault(task.task_id, []).append(ErrorDetail(
                code=ErrorCode.JUDGE_PARTIAL_VALIDATION_ERROR.value,
                node="batch_judge", tool="single_judge_adapter", retryable=False,
...
def _safe_provider_metadata(response: Any) -> dict[str, Any]:
    ...
    allowed = {
        "id", "model", "model_name", "model_provider", "stop_reason",
...
def extract_llm_payload(response: Any, preview_chars: int = 6000) -> ExtractedLLMPayload:
    ...
    if response is None:
        ...
        if isinstance(response, list):
            payload = response
        elif any(key in response for key in ("results", "items", "judgements", "judgments", "data", "candidate_id", "candidateId", "evidence_id")):
            payload = response
    ...
    if isinstance(payload, str):
        ...
        length = len(preview)
    empty = payload is None or (isinstance(payload, str) and not payload.strip()) or payload == [] or payload == {}
    return ExtractedLLMPayload(
        payload=payload, raw_response_type=response_type,
        raw_response_length=length, raw_response_preview=preview,
        provider_response_metadata=metadata, empty=empty, recognized=recognized,
    )

_RELATION_NORMALIZE_MAP: dict[str, EvidenceRelation] = {
    "SUPPORT": EvidenceRelation.SUPPORT, "SUPPORTED": EvidenceRelation.SUPPORT,
    "SUPPORTS": EvidenceRelation.SUPPORT, "support": EvidenceRelation.SUPPORT,
    "supports": EvidenceRelation.SUPPORT, "supported": EvidenceRelation.SUPPORT,
    "REFUTE": EvidenceRelation.REFUTE, "REFUTED": EvidenceRelation.REFUTE,
    "REFUTES": EvidenceRelation.REFUTE, "refute": EvidenceRelation.REFUTE,
    "refuted": EvidenceRelation.REFUTE, "refutes": EvidenceRelation.REFUTE,
    "SUPPLEMENT": EvidenceRelation.SUPPLEMENT, "SUPPLEMENTARY": EvidenceRelation.SUPPLEMENT,
    "supplement": EvidenceRelation.SUPPLEMENT, "supplementary": EvidenceRelation.SUPPLEMENT,
    "NEUTRAL": EvidenceRelation.NEUTRAL, "IRRELEVANT": EvidenceRelation.NEUTRAL,
    "UNRELATED": EvidenceRelation.NEUTRAL, "UNKNOWN": EvidenceRelation.NEUTRAL,
    "neutral": EvidenceRelation.NEUTRAL, "irrelevant": EvidenceRelation.NEUTRAL,
    "unrelated": EvidenceRelation.NEUTRAL, "unknown": EvidenceRelation.NEUTRAL,
}
...
def parse_json_or_python_literal(payload: Any) -> tuple[Any | None, list[dict[str, Any]]]:
    ...
    for value in attempts:
        ...
        if repaired == value:
            continue
        try:
            return json.loads(repaired), [{"stage": "parse", "warning": True, "reason": "已保守移除尾随逗号"}]
        except json.JSONDecodeError:
            continue
    reason = failures[0] if failures else "no parseable object"
    return None, [{"stage": "parse", "reason": f"响应无法解析为 JSON 或安全 Python literal: {reason}"}]
...
def normalize_batch_judge_container(parsed: Any) -> tuple[list[Any], list[dict[str, Any]]]:
    ...
    if not isinstance(parsed, dict):
        return [], [{"stage": "container", "reason": f"Judge 顶层必须是对象或数组，实际为 {type(parsed).__name__}"}]
    for key in ("results", "items", "judgements", "judgments", "data"):
        if key in parsed:
            value = parsed[key]
            if isinstance(value, list):
                return value, []
            return [], [{"stage": "container", "reason": f"外层字段 {key} 必须是数组，实际为 {type(value).__name__}"}]
    ...
    if item_keys.intersection(parsed) and relation_keys.intersection(parsed):
        return [parsed], []
    return [], [{"stage": "container", "reason": "无法识别 Batch Judge 外层对象，未发现 results/items/judgements/judgments/data"}]


def normalize_judge_item(item: Any, index: int) -> tuple[dict[str, Any] | None, list[dict[str, Any]]]:
    if isinstance(item, BaseModel):
        item = item.model_dump(mode="json")
    if isinstance(item, (list, tuple)):
        if len(item) < 5 or len(item) > 9:
            return None, [{
                "stage": "item", "index": index,
    ...
    if not isinstance(item, dict):
        return None, [{"stage": "item", "index": index, "reason": f"Judge item 必须是对象，实际为 {type(item).__name__}"}]
    nested = item.get("judgement") or item.get("judgment")
    if isinstance(nested, BaseModel):
        nested = nested.model_dump(mode="json")
    if isinstance(nested, dict):
        item = {**nested, **{key: value for key, value in item.items() if key not in {"judgement", "judgment"}}}
    aliases = {
    ...
    if str(raw_relation).strip().upper() not in _RELATION_NORMALIZE_MAP:
        warnings.append({"stage": "item", "index": index, "candidate_id": normalized.get("candidate_id"), "warning": True, "reason": f"未知 relation {raw_relation!r}，已降级为 NEUTRAL"})
    ...
    try:
        ...
        warnings.append({"stage": "item", "index": index, "candidate_id": normalized.get("candidate_id"), "warning": True, "reason": f"confidence {raw_confidence!r} 无法转换，使用 0.5"})
...
def map_judge_results_to_candidates(
    ...
    for idx, item in enumerate(parsed_items):
        try:
            # 提取 candidate_id
            cid = item.get("candidate_id") or item.get("evidence_id") or item.get("id") or item.get("candidateId")
            ...
            if tid and cid:
                ...
                if len(matches) == 1:
                    ...
                    errors.append({
                        "stage": "mapping", "index": idx, "candidate_id": cid,
            ...
            if key not in candidates:
                errors.append({
                    "stage": "mapping", "index": idx, "task_id": str(tid) if tid else None,
            ...
            if key in output:
                errors.append({
                    "stage": "mapping", "index": idx, "task_id": key[0], "candidate_id": key[1],
            ...
            # 解析 quoted_spans
            quoted_spans = item.get("quoted_spans", item.get("quotes", item.get("quote", item.get("evidence_quotes", []))))
            ...
            reason = str(item.get("reason", item.get("explanation", item.get("rationale", ""))) or "")
            ...
            if claims_by_task is not None:
                ...
                for raw_claim in raw_claim_results:
                    ...
                    if claim_id not in expected_claims or claim_id in seen_claim_ids:
                        errors.append({
                            "stage": "claim_mapping", "task_id": key[0], "candidate_id": key[1],
                    ...
                    if re.search(r"无法直接(?:支持|确认|反驳|否定)|cannot directly (?:support|refute)", claim_reason, re.I):
                    ...
                    if not isinstance(claim_quotes, list):
                        claim_quotes = []
                    claim_quotes = [str(value) for value in claim_quotes if isinstance(value, (str, int, float)) and str(value).strip()]
                    raw_neutral = raw_claim.get("neutral_reason")
                    try:
                        claim_neutral = NeutralReason(str(raw_neutral).upper()) if raw_neutral else None
                    except ValueError:
                    ...
                    if claim_relation == EvidenceRelation.NEUTRAL:
                        ...
                        try:
                            claim_directness = max(0.0, min(1.0, float(raw_claim.get("directness", 0.0))))
                        except (TypeError, ValueError):
                            claim_directness = 0.0
                        grounded = validate_judgement(candidate, JudgeResult(
                    ...
                    normalized_claim_slots = {
                        str(slot): value if isinstance(value, dict) else {
                            "value": value,
                            "quote": claim_quotes[0] if claim_quotes else "",
                        }
                        for slot, value in (raw_slot_evidence.items() if isinstance(raw_slot_evidence, dict) else [])
                    }
                    ...
                    if not isinstance(claim_numeric_facts, list):
                        claim_numeric_facts = []
                    claim_numeric_facts = [value for value in claim_numeric_facts if isinstance(value, dict)]
                    claim_results.append(ClaimJudgeResult(
                        claim_id=claim_id,
            ...
            if claim_results:
                supported_claim_ids = [row.claim_id for row in claim_results if row.relation == EvidenceRelation.SUPPORT]
                refuted_claim_ids = [row.claim_id for row in claim_results if row.relation == EvidenceRelation.REFUTE]
                non_neutral = [row for row in claim_results if row.relation != EvidenceRelation.NEUTRAL]
                ...
                quoted_spans = list(dict.fromkeys(span for row in non_neutral for span in row.quoted_spans))

            judge_result = JudgeResult(
                relation=relation, confidence=confidence, directness=directness,
                reason=reason, quoted_spans=quoted_spans,
                supported_claim_ids=supported_claim_ids,
                refuted_claim_ids=refuted_claim_ids,
                covered_slots=covered_slots, missing_slots=missing_slots,
                slot_evidence=slot_evidence, numeric_facts=numeric_facts,
            ...
            errors.append({
                "stage": "item", "index": idx,
                "task_id": item.get("task_id"), "candidate_id": item.get("candidate_id", "unknown"),
    ...
    for task_id, candidate_id in candidate_keys:
        if (task_id, candidate_id) not in output:
            errors.append({
                "stage": "mapping", "task_id": task_id, "candidate_id": candidate_id,
...
def parse_batch_judge_response(
    ...
    output, mapping_errors = map_judge_results_to_candidates(normalized_items, candidates, claims_by_task)
...
def _trim_candidate_content(candidate: EvidenceCandidate, target_text: str, max_chars: int) -> str:
    ...
    ranked = BM25Retriever(text_getter=lambda row: f"{candidate.title} {row[1]}").retrieve(target_text, indexed, 1)
    if not ranked:
...
def to_evidence(candidate: EvidenceCandidate, judgement: JudgeResult) -> EvidenceItem:
    ...
    return EvidenceItem(
        evidence_id=f"ev-{evidence_key[:20]}", task_id=candidate.task_id,
        source_type=candidate.source_type, source_name=candidate.source_name,
        source_ref=candidate.source_ref, title=candidate.title, content=candidate.content,
        quoted_spans=judgement.quoted_spans, snippet_only=candidate.snippet_only,
        relation=judgement.relation, judge_confidence=judgement.confidence,
        scores=EvidenceScores(relevance=candidate.rerank_score, directness=judgement.directness),
        covered_slots=judgement.covered_slots, missing_slots=judgement.missing_slots,
        reason=judgement.reason, content_fingerprint=key,
        source_evidence_fingerprint=source_fingerprint,
        metadata={**candidate.metadata, "slot_evidence": judgement.slot_evidence,
